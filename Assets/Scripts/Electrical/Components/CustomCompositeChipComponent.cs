...
{
    /// <summary>
    /// Custom Composite Chip model generated from CircuitJS1 reference file CustomCompositeChipElm.java.
    /// Simplified for Unity breadboard integration with stable stamping/step behavior.
    /// TODO(parity): compare against https://github.com/pfalstad/circuitjs1/blob/master/src/com/lushprojects/circuitjs1/client/CustomCompositeChipElm.java for full behavioral parity.
    /// </summary>
    public class CustomCompositeChipComponent : ElectricalComponentBase
    {
...
[SerializeField] private float outputImpedanceOhms = 1000f;
        ...
        [Tooltip("Terminal index 4")]
        [SerializeField] private int terminal4 = 4;

        [Tooltip("Terminal index 5")]
        [SerializeField] private int terminal5 = 5;

        [Tooltip("Terminal index 6")]
        [SerializeField] private int terminal6 = 6;

        [Tooltip("Terminal index 7")]
        [SerializeField] private int terminal7 = 7;
        ...
        public override ElectricalComponentKind Kind => ElectricalComponentKind.CustomCompositeChip;

        public override int TerminalCount => 8;
        ...
        public override void Stamp(CircuitMatrix matrix)
        {
            ...
            matrix.StampConductance(NodeOrDefault(terminal0), NodeOrDefault(terminal1), conductance);
                        ...
                        if (HasValidNodes(terminal0, terminal4)) matrix.StampConductance(NodeOrDefault(terminal0), NodeOrDefault(terminal4), conductance * 0.25f);
                        if (HasValidNodes(terminal0, terminal5)) matrix.StampConductance(NodeOrDefault(terminal0), NodeOrDefault(terminal5), conductance * 0.25f);
                        if (HasValidNodes(terminal0, terminal6)) matrix.StampConductance(NodeOrDefault(terminal0), NodeOrDefault(terminal6), conductance * 0.25f);
                        if (HasValidNodes(terminal0, terminal7)) matrix.StampConductance(NodeOrDefault(terminal0), NodeOrDefault(terminal7), conductance * 0.25f);
        }
